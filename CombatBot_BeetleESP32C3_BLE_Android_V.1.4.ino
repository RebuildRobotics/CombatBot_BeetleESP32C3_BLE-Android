...
void monitorVoltage() {
  ...
  // Low voltage guard
  if (useLowVoltageGuard && !batteryEmpty && voltageLevel <= lowVoltageLimit) {
    batteryEmpty = true;
    failsafe(true);
  } else if (useLowVoltageGuard && batteryEmpty && voltageLevel > lowVoltageLimit && deviceConnected) {
    batteryEmpty = false;
    failsafe(false);
  }
